...
# ---------------------------------------------------------------------------
# Hero background processing (HomePageContent.hero_background)
# ---------------------------------------------------------------------------

HERO_SIZE_LARGE = 1920  # main display size
HERO_SIZE_THUMB = 960  # preload / mobile srcset
HERO_WEBP_QUALITY = 85
HERO_WEBP_QUALITY_THUMB = 82


def _hero_unique_rel_path(suffix, original_name, sample_bytes):
    h = hashlib.sha256(sample_bytes[:65536]).hexdigest()[:12]
    base = "hero"
    if original_name:
        base = (Path(str(original_name)).stem[:30] or "hero").replace(" ", "_")
    ts = int(time.time() * 1000)
    nonce = secrets.token_hex(4)
    return f"home/hero/{suffix}/{base}_{h}_{ts}_{nonce}.webp"


def process_hero_image(ufile):
    """Validate + transcode the home-page hero into two WebP variants.

    Returns a dict with two ``ContentFile`` instances ready to be assigned to
    ``HomePageContent.hero_background`` (1920w) and
    ``HomePageContent.hero_background_thumb`` (960w).
    ...
    raw = ufile.read()
    sample = raw
    name = getattr(ufile, "name", "") or "hero"
    ...
    large_path = _hero_unique_rel_path("large", name, sample)
    large_file = ContentFile(large_data, name=large_path)

    thumb_data, thumb_w, thumb_h = _save_webp(
        img,
        HERO_SIZE_THUMB,
        quality=HERO_WEBP_QUALITY_THUMB,
        resample=Image.Resampling.BILINEAR,
    )
    thumb_path = _hero_unique_rel_path("thumb", name, sample)
    thumb_file = ContentFile(thumb_data, name=thumb_path)
